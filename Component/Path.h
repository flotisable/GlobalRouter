...
class Path
{
  public:
    ...
    inline const RoutingRegion* belongRegion() const;
    inline vector<Point>&       path        ();
    inline const vector<Point>& path        () const;
    inline int                  layer       () const;

    inline void setBelongRegion ( const RoutingRegion *region );
    inline void setLayer        ( int                 layer   );

  private:

    const RoutingRegion *mBelongRegion;
    vector<Point> mPath;
    int           mLayer;
};
...
inline Path::Path( int layer ) : mBelongRegion( NULL ) { setLayer( layer ); }

inline const RoutingRegion* Path::belongRegion() const  { return mBelongRegion; }
...
inline void Path::setBelongRegion ( const RoutingRegion *region ) { mBelongRegion = region; }
